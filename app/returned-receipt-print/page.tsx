  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      ...
      if (!parsed || parsed._error) {
        ...
        try {
          const localData = localStorage.getItem('ar-invoice-print-data');
          if (localData) {
  ...
  if (!data) {
    return (
      <div style={{ color: 'red', textAlign: 'center', padding: 40 }}>
        ...
        <pre style={{ color: '#333', background: '#eee', padding: 12, marginTop: 16, maxWidth: 600, overflowX: 'auto' }}>localStorage: {typeof window !== 'undefined' ? localStorage.getItem('ar-invoice-print-data') : ''}</pre>
      </div>
    );
  }
